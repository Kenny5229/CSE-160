  ...
  void main() {
    ...
    if (u_SpotOn) {
      ...
      if (spotCos > u_SpotCutoff) {
        ...
        float t = smoothstep(u_SpotCutoff, 1.0, spotCos);
...
let u_LightPos, u_LightColor, u_ViewPos;
...
const TEX_FILES = ["sky.jpg", "grass.jpg", "wood.png", "wood.png"];
...
function main() {
  ...
  initSphereBuffers(gl, a_Position, a_UV, a_Normal);
...
// ===== Lighting helpers =====
function updateLightAnim() {
  // gentle orbit around the center unless user is dragging sliders (sliders still override because they write g_lightPos)
  g_lightAngle += 0.01;
  // If you don't want animation, comment the next 3 lines.
  const r = 10;
  g_lightPos[0] = WORLD_SIZE/2 + r * Math.cos(g_lightAngle);
  g_lightPos[2] = WORLD_SIZE/2 + r * Math.sin(g_lightAngle);
}
...
function hookLightingUI() {
  ...
  if (!btnLight) return; // if HUD not present
  ...
  function updatePos() {
    // Stop orbit when user uses sliders by freezing angle at current, but keep y from slider
    g_lightAngle = g_lightAngle; 
